...
def free_bacon(score):
    ...
    # BEGIN PROBLEM 2
    zero_score = (score // 10) * 2 - (score % 10)
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say(score0, score1):
        if who == 0:
            ...
            return announce_highest(who, max(gain, previous_high), score0)
        if who == 1:
            gain = score1 - previous_score
            if gain > previous_high:
                print(gain, "point(s)! That's the biggest gain yet for Player", who)
            return announce_highest(who, max(gain, previous_high), score1)
    return say
...
def make_averaged(fn, num_samples=1000):
    ...
    def strategy(*args):
        func_sum = 0
        num_times = 0
        while num_times < num_samples:
            func_sum += fn(*args)
            num_times += 1
        return func_sum / num_samples
    return strategy
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    for num in range(1,11):
        num_avg = make_averaged(roll_dice, num_samples)
        result = num_avg(num, dice)
        if result > max_num_avg:
            max_num_avg = result
            highest = num
    return highest
    # END PROBLEM 9
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if score > opponent_score:
        if is_swap(bacon_score, opponent_score) == True:
            return num_rolls
