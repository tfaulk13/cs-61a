...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(scheme_eval(first, env), MacroProcedure):
            return scheme_eval(scheme_eval(first, env).apply_macro(rest, env), env)
        operands = rest.map((lambda x: scheme_eval(x, env)))
        return scheme_apply(scheme_eval(first, env), operands, env)
        # END PROBLEM 5
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(vals) != len(formals):
            raise SchemeError('Number of formals does not equal number of vals.')
        child = Frame(self)
        while len(formals) > 0:
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        4
        """
        if not scheme_listp(args): # args is a Scheme list represented as a Pair object
            raise SchemeError('arguments are not in a list: {0}'.format(args))
        # Convert a Scheme list to a Python list
        python_args = []  #
        while args is not nil: # convert the scheme list into a python list of arguments
...
def make_let_frame(bindings, env):
    ...
    new_frame = env.make_child_frame(formals, vals)
    return new_frame
